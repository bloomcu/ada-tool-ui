<template>
  <LayoutWithSidebar>
      ...
      <ul role="list" class="divide-y divide-gray-100">
        <li v-for="scan in scanStore.scans" :key="scan.id" class="relative flex justify-between gap-x-6 px-4 py-5 hover:bg-gray-50 sm:px-6">
          <div class="min-w-0 flex-auto">
            <p class="text-base font-medium leading-6 text-gray-900">{{ scan.domain }}</p>
            <p class="mt-1 text-sm leading-5 text-gray-500">{{ scan.title }}</p>
    ...
    <div v-else class="text-center bg-slate-50 rounded-2xl py-12 px-2">
      ...
      <p class="mt-1 text-gray-500">Get started by running a scan.</p>
      <!-- <AppButton @click="" class="mt-4">Run scan</AppButton> -->
    </div>
